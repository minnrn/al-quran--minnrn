  ...
  const getAyat = async () => {
    ...
    try {
      const request = await fetch(
        `https://api.quran.gading.dev/surah/${id}/${ayat}`
      );
      const response = await request.json();
          ...
          <div className="px-7 pt-6" style={{ overflow: "hidden" }}>
            <div className="text-center">
              <p className="text-gray-800 font-semibold text-2xl nama-surah">
                <span>{detailSurat?.name?.short}</span>
              </p>
              ...
              {!loading && id && detailAyat?.number?.inSurah && (
                <div className="flex justify-around items-center">
                  <Link
                    to={`/surah/${id}/${
                      detailAyat?.number?.inSurah - 1 === 0
                        ? 1
                        : detailAyat?.number?.inSurah - 1
                    }`}
                    className={`${
                      detailAyat?.number?.inSurah - 1 < 1 ? "hidden" : "inline"
                    }`}
                  >
              ...
              <p className="text-gray-500 desc hidden">
              ...
              <div className="py-6 border-b flex-column gap-4 items-start justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-slate-800 text-justify mb-2">
                      {detailAyat?.tafsir?.id?.long}
                    </p>
                  </div>
                  <audio
